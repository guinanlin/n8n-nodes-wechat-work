...
const UserCreateOperate: ResourceOperations = {
	name: '创建成员',
	value: 'user:create',
	...
	async call(this: IExecuteFunctions, index: number): Promise<IDataObject> {
		const departments = NodeUtils.getNodeFixedCollection(
			this.getNodeParameter('department', index) as IDataObject,
			'values',
		);

		const directLeaders = NodeUtils.getNodeFixedCollectionList(
			this.getNodeParameter('direct_leader', index) as IDataObject,
			'values',
			'direct_leader',
		);
